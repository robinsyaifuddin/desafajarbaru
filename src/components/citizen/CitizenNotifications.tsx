...
const CitizenNotifications = () => {
  ...
  const getNotificationBadge = (type) => {
    const badges = {
      service: { text: 'Layanan', color: 'bg-emerald-500' },
      complaint: { text: 'Pengaduan', color: 'bg-orange-500' },
      announcement: { text: 'Pengumuman', color: 'bg-blue-500' },
      poll: { text: 'Polling', color: 'bg-purple-500' }
    };
    
    return badges[type] || { text: 'Notifikasi', color: 'bg-gray-500' };
  };
  ...
  const NotificationCard = ({ notification }) => {
    ...
    return (
      <Card className={`p-4 ${!notification.isRead ? 'border-l-4 border-l-emerald-500 bg-emerald-50/30' : 'border-l-4 border-l-gray-200'}`}>
